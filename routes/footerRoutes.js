...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      if (req.files?.logo?.[0]) {
        const logoUrl = await uploadToCloudinary(req.files.logo[0].buffer, "footer/logo");
        settings.logoUrl = logoUrl;
      }

      if (req.files?.linkedinIcon?.[0]) {
        const iconUrl = await uploadToCloudinary(
          req.files.linkedinIcon[0].buffer,
          "footer/icons"
        );
        settings.linkedinIconUrl = iconUrl;
      }

      // Update text fields (allow empty string)
